...
LLM은 복잡한 인공지능 모델이지만, 그 핵심 기반 기술(아키텍처)과 두 가지 주요 학습 과정으로 나누어 이해할 수 있다.
...
graph LR
    subgraph "트랜스포머 아키텍처 (Transformer Architecture)"
        ...
        %% 디코더 파트
        H[출력 시퀀스/이전 토큰] --> I(임베딩 & 위치 인코딩);
        I --> E["디코더 스택(Decoder Stack)"];
        E -- "Masked Self-Attention" --> E;
        
        %% 교차 어텐션 (인코더 -> 디코더 흐름)
        D -. "Encoder-Decoder Attention" .-> E;
    ...
    style H fill:#DDEBF7,stroke:#367C9D,stroke-width:2px;
    style I fill:#E2F0D9,stroke:#548235,stroke-width:2px;
```

> **참고:** 오리지널 트랜스포머는 위 그림처럼 인코더와 디코더로 구성되지만, ChatGPT의 근간이 되는 GPT(Generative Pre-trained Transformer) 계열 모델들은 텍스트 생성에 특화된 **'디코더(Decoder)'** 영역만을 집중적으로 확장하여 사용하는 **Decoder-only 아키텍처**입니다.
...
#### 1.3.3. 창발적 능력 (Emergent Abilities)

LLM이 특정 규모 이상으로 커지면, 사전 학습 단계에서는 명확히 예측하기 어려웠던 새로운 능력들이 나타나는 경우가 있다. 이를 '창발적 능력(Emergent Abilities)'이라고 부른다. 예를 들어, 사칙연산, 추론, 특정 프로그래밍 언어 이해 등은 모델의 크기가 커짐에 따라 갑자기 향상되거나 발현되는 능력들이다. 이는 LLM 연구의 중요한 발견 중 하나이며, 모델의 잠재력을 보여주는 지표이다.
...
LLM의 텍스트 생성과 분석 능력을 결합하여, 전문가를 보조하는 역할로 특정 산업군에 광범위하게 적용될 수 있다.
...
이러한 **RAG 패턴을 쉽고 효율적으로 구현할 수 있도록 돕는 대표적인 프레임워크가 바로 LangChain**이며, 본 과정에서는 LangChain을 활용하여 실무 수준의 AI 서비스를 개발하는 방법을 학습할 것이다.
...
graph TD
    subgraph "RAG를 통한 개선"
        D[사용자 질의] --> E(검색기/리트리버);
        D -. "원래 질의 포함" .-> I;
        E --> F[외부 지식 저장소 최신 정보 및<br>내부 데이터];
        F --> G{관련 정보 검색};
        G --> H[검색된 정보];
        H --> I(LLM);
