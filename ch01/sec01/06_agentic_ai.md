...
기존의 대규모 언어 모델(LLM)은 사용자의 프롬프트에 응답하는 강력한 도구이지만, 근본적인 한계를 지닌다. 이는 사용자의 명령에만 반응하는 수동성(Passivity)과 대화가 종료되면 작업이 중단되는 지속성 부족(Lack of Persistence)이다.

이러한 한계를 극복하기 위해, 목표를 부여받으면 자율적으로 계획을 수립하고, 도구를 사용하며, 스스로 행동을 수정하는 시스템, 즉 'AI 에이전트' 패러다임이 부상하고 있다. 본 문서는 고전적 AI 에이전트의 개념과 LLM 기반의 '에이젠틱 AI(Agentic AI)' 패러다임을 명확히 구분하고, 그 핵심 아키텍처와 도전 과제를 분석한다.

### 2\. 용어의 정의: AI 에이전트 vs 에이젠틱 AI

개념의 혼재를 피하기 위해 두 용어의 정의를 명확히 할 필요가 있다.

#### 2.1. AI 에이전트 (AI Agent)

AI 에이전트는 환경을 인식하고(Perceive), 추론하며(Reason), 목표를 달성하기 위해 행동(Act)하는 모든 컴퓨팅 개체를 포괄하는 **광의의 용어**이다.
...
#### 2.2. 에이젠틱 AI (Agentic AI)

에이젠틱 AI는 이 광범위한 에이전트 개념 중에서도, **LLM을 핵심 추론 엔진(두뇌)으로 활용**하여 고도의 자율성과 복잡한 계획 수립 능력을 갖춘 **최신 패러다임**을 지칭한다.

- **핵심 특징:** LLM 기반의 복잡한 **작업 분해(Task Decomposition)**, **도구 사용(Tool Use)**, 그리고 자기 성찰(Self-reflection)을 통해 복잡하고 장기적인 목표를 자율적으로 수행한다.
- **개념적 관계:** 에이젠틱 AI는 AI 에이전트의 가장 진보된 하위 집합이다. '에이젠틱(Agentic)'하다는 것은 이러한 LLM 기반의 자율적 속성을 의미한다.

'멀티 에이전트 시스템(Multi-Agent System)'은 에이젠틱 AI의 정의 자체가 아니라, 여러 에이젠틱 AI가 협업하도록 구성한 **고급 아키텍처 구현 방식** 중 하나이다.
...
**핵심**: "에이젠틱"은 **작동 방식**을, "에이전트"는 **실체**를 의미한다.
...
**에이젠틱 AI**는 이러한 AI 에이전트들을 여러 개 조합하고 협업시키는 것이다.

에이젠틱 AI 시스템은 단일 LLM을 넘어 여러 구성 요소가 유기적으로 결합하여 작동한다.

#### 3.1. LLM: 시스템의 두뇌 (추론 엔진)
...
#### 3.2. 추론 프레임워크: ReAct와 Plan-and-Act
...
#### 3.3. 메모리 시스템: RAG (Retrieval-Augmented Generation)
...
#### 3.4. 행동 능력: 함수 호출 (Function Calling)
