...
```
채팅형 AI (Passive)
└─ 질문 → 답변 (단일 응답 / One-shot)

AI 에이전트 (The Doer)
└─ 특정 목표 수행 → 스스로 도구 사용 → 결과 도출

에이젠틱 AI (The How / Workflow)
└─ 복잡한 목표 → 계획 → 실행 → 자기 성찰(Reflection) → 반복 개선
```
...
```
AI 에이전트 (Who: 실체)
    └─ 수행 주체: "특정 역할을 가진 지능형 개체"

에이젠틱 워크플로우 (How: 방식)
    ├─ Single Agent: 혼자서 "계획 → 실행 → 검토" 루프 수행
    └─ Multi-Agent: 여러 에이전트가 협력하여 "복잡한 문제 해결" 루프 수행
