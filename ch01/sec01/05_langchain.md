...
LLM 애플리케이션을 만드는 대표적인 세 가지 도구인 n8n, Opal, 그리고 랭체인은 모두 위에서 언급한 아키텍처를 구현한다. 하지만 그 형태와 목적에는 분명한 차이가 있다.
...
- **Opal**: 최근 주목받는 LLM 기반 노코드 플랫폼으로, 자연어 프롬프트와 시각적 편집기를 통해 '미니 앱'을 자동으로 생성해준다. 사용자가 로직을 설명하면 Opal이 내부적으로 위에 언급된 아키텍처 요소를 결합하여 하나의 애플리케이션으로 만들어낸다.
