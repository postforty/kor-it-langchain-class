...
### 2.6. 데이터 흐름 요약 (Inference Pipeline)

ChatGPT의 응답 과정을 일련의 단계로 요약하면 다음과 같습니다:

1. **입력 & 토큰화**: `사용자 질문 텍스트 입력` ➔ `전처리 필터 적용` ➔ `토큰 단위로 분할`
2. **이해 & 추론**: `임베딩(수치화)` ➔ `트랜스포머 인코더/디코더 처리` ➔ *(필요시: 외부 시스템/RAG 연동으로 정보 보강)* ➔ `다음 토큰 확률 분포 생성` ➔ `확률 기반 토큰 샘플링 선택`
3. **출력 변환**: `선택된 토큰 시퀀스를 인간의 문자로 디코딩` ➔ `마크다운 포맷팅 등 후처리` ➔ `최종 응답 화면 출력`
