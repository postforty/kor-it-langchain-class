...
- **트랜스포머의 혁신**: 순환 유닛이 없어 RNN보다 훨씬 빠른 학습 시간
  - 모든 위치의 정보를 동시에 계산
  - GPU 최적화에 매우 유리
  > **참고:** 트랜스포머의 병렬 처리는 '모델 학습(Training)' 단계의 가장 큰 장점입니다. 반면 챗봇이 답변을 '생성(Inference)'할 때는 문맥 예측 방식에 따라 여전히 이전 단어를 바탕으로 다음 단어를 하나씩 순차적(Autoregressive)으로 생성해 냅니다.
...
- **RNN/LSTM**: 기본 RNN은 그래디언트 소실 문제가 심각하며, LSTM이 이를 완화하긴 했으나 여전히 구조적 한계로 인해 매우 긴 시퀀스 처리 어려움
