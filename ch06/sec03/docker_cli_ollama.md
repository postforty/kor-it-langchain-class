...
```bash
docker run -d -v C:/docker_data/ollama:/root/.ollama -p 11434:11434 --name ollama ollama/ollama
```
...
| `-v C:/docker_data/ollama:/root/.ollama` | 호스트 PC의 디렉터리를 컨테이너 내부 디렉터리에 연결하는 볼륨 마운트 설정(데이터 영구 저장) |
