...
### 2. 벡터 공간에서의 유사성 및 거리 계산

임베딩된 벡터들은 고차원 공간에서 의미적으로 유사한 텍스트끼리 가까이 위치한다. 이를 측정하기 위해 주로 **코사인 유사도**와 **유클리드 거리**가 사용된다.

#### ① 코사인 유사도 (Cosine Similarity)
가장 널리 사용되는 방식으로, 두 벡터 사이의 **각도**를 측정한다. 텍스트의 길이나 크기보다는 **방향(의미)**이 얼마나 일치하는지에 집중한다.

```
코사인 유사도 = (벡터A · 벡터B) / (|벡터A| × |벡터B|)

결과값 (범위: -1 ~ 1):
- **1에 가까울수록**: 의미적으로 매우 유사함 (두 벡터의 방향이 일치)
- **0에 가까울수록**: 두 텍스트 간의 연관성이 없음 (두 벡터가 서로 직교)
- **-1에 가까울수록**: 의미적으로 반대됨 (이론적인 반대 방향)

> **※ 실무 참고:** 자연어 임베딩에서 두 문장이 완전히 반대(-1)인 경우는 극히 드물며, 대부분의 검색 시스템에서는 **0에서 1 사이의 유사도 값**을 주로 활용하게 됩니다.
```

#### ② 유클리드 거리 (Euclidean Distance / L2 Distance)
두 벡터 사이의 **직선 거리**를 측정한다. 값이 작을수록 두 벡터가 가깝고 유사하다는 것을 의미한다.

```
유클리드 거리 = √Σ(Ai - Bi)²

결과값 (범위: 0 ~ ∞):
- **0에 가까울수록**: 두 벡터가 물리적으로 가까움 (매우 유사)
- **값이 커질수록**: 두 벡터 사이의 거리가 멀어짐 (관련성 낮음)
```

#### ③ 내적 (Dot Product / Inner Product)
두 벡터의 대응하는 요소들을 곱한 뒤 모두 합산한 값이다. 벡터의 **방향**뿐만 아니라 **크기(강도)**까지 고려한다.

```
내적 = Σ(Ai × Bi)

특징:
- 벡터의 길이가 길수록(단어가 많거나 강조될수록) 값이 커진다.
- 벡터가 정규화(길이가 1로 고정)되어 있다면 코사인 유사도와 결과가 같다.
- 속도가 매우 빨라 대규모 검색 엔진에서 선호된다.
```

#### ④ 맨해튼 거리 (Manhattan Distance / L1 Distance)
각 차원별 차이의 절대값을 모두 더한 거리이다. 격자 형태의 도로에서 길을 찾는 것과 같아 '택시 거리'라고도 불린다.

```
맨해튼 거리 = Σ|Ai - Bi|

특징:
- 차원이 매우 높은 경우 유클리드 거리보다 성능이 좋을 때가 있다.
- 이상치(Outlier)에 대해 유클리드 거리보다 덜 민감하다.
```

#### ⑤ 자카드 유사도 (Jaccard Similarity) - 번외
임베딩 벡터가 아닌 **원본 텍스트의 단어 집합**을 비교할 때 사용한다. 두 문장에 공통된 단어가 얼마나 많은지를 측정한다.

```
자카드 유사도 = (교집합의 크기) / (합집합의 크기)

특징:
- 의미보다는 '정확한 단어의 일치'가 중요할 때 사용한다. (키워드 기반 검색)
```

#### 어떤 것을 사용해야 할까?
- **코사인 유사도**: 문서의 길이가 달라도 **주제(맥락)**가 유사한 것을 찾을 때 (RAG 표준)
- **내적 (Dot Product)**: 벡터의 **강도(중요도)**를 함께 고려해야 하거나, 연산 속도가 매우 중요한 경우
- **유클리드/맨해튼**: 데이터의 **수치적 차이** 자체가 중요한 경우
- **자카드**: 키워드 기반의 **단어 중복**을 확인하고 싶을 때

> **결론:** 대부분의 LangChain 및 RAG 프로젝트에서는 **코사인 유사도**를 기본으로 시작하며, 대규모 시스템이나 특정 모델(정규화되지 않은 모델)의 경우 **내적**을 선택하기도 합니다.
