...
# [정답] langchain_ollama 모듈 임포트
from langchain_ollama import OllamaEmbeddings
from langchain_ollama import ChatOllama # 또는 OllamaLLM
...
st.title("PDF 기반 QA봇 (Ollama 정답)")
st.caption("정답: Ollama Embeddings + Ollama LLM")
...
def create_chain(retriever, prompt_filepath):
    ...
    # [정답] Ollama LLM 모델 설정
    llm = ChatOllama(
        model="qwen3:latest", # 또는 사용자가 보유한 다른 모델 (예: llama3.1, eeve:10.8b-chat 등)
        temperature=0,
