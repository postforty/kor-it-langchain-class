...
2.  **프롬프트 (Prompt)**: 검색된 정보를 바탕으로 언어 모델이 답변을 생성할 수 있도록 최종 입력값을 구성하는 단계이다. 보통 **"프롬프트(Prompt) = 지시사항(Instruction) + 검색된 결과(Context) + 사용자 질문(Question)"**의 형태로 구성된다. 여기서 **Context**는 이전 대화의 맥락(History)이 아니라, 검색기(Retriever)가 찾아온 **관련 정보나 문서 조각**을 의미한다.
