...
class PageAnalyzer:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        # 노드 추가
        graph_builder.add_node("generate_selector", self._generate_selector_node)
        graph_builder.add_node("extract_html_elements", self._extract_html_elements_node)
    ...
    def _generate_selector_node(self, state: PageAnalyzerState) -> PageAnalyzerState:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", 
             """당신은 사용자의 요청과 제공된 HTML을 분석하여, 웹페이지에서 특정 요소를 찾는 데 가장 적합한 CSS 셀렉터를 반환하는 전문가입니다.
...
반드시 규칙에 따라 CSS 셀렉터 문자열 또는 'None'만 반환해야 합니다."""),
    ...
    def _extract_html_elements_node(self, state: PageAnalyzerState) -> PageAnalyzerState:
        ...
        try:
            ...
            if not elements:
                print(f"🤖 Page Analyzer: 셀렉터 '{selector}'에 해당하는 요소를 찾을 수 없습니다.")
...
if __name__ == '__main__':
    ...
    <body>
        <header>
            <h1 id="main-title">나의 블로그</h1>
        </header>
        <main>
            <article class="blog-post">
                <h2>첫 번째 포스트</h2>
                <p>이것은 첫 번째 블로그 포스트의 내용입니다.</p>
            </article>
            <div class="search-container">
                <label for="search-box">검색:</label>
                <input type="search" id="search-box" name="query" placeholder="검색어를 입력하세요...">
                <button class="search-button">찾기</button>
            </div>
            <article class="blog-post">
                <h2>두 번째 포스트</h2>
                <p>이것은 두 번째 블로그 포스트의 내용입니다.</p>
            </article>
        </main>
    </body>
    </html>
    ...
    user_query_for_test = "검색창을 찾아서 알려줘"
    
    # --- Page Analyzer 실행 ---
    analyzer = PageAnalyzer()
    final_result_state = analyzer.run(query=user_query_for_test, html_content=mock_html_for_test)
    
    # --- 결과 출력 ---
    print("\n" + "="*50)
    print("✅ 최종 분석 결과 (슈퍼바이저가 받게 될 데이터):")
    
    # 보기 좋게 출력하기 위해 JSON 라이브러리 활용
    import json
    print(json.dumps(final_result_state, indent=2, ensure_ascii=False))
    print("="*50)